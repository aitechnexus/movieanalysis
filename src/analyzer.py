...
class MovieAnalyzer:
    ...
    def get_top_movies(
        ...
        # Step 1: Calculate basic statistics for each movie
        # Group ratings by movieId and compute:
        # - v: vote count (number of ratings)
        # - R: mean rating for the movie
        # - rating_std: standard deviation of ratings (measure of consensus)
        stats = (
            self.ratings.groupby("movieId")
            .agg(
                v=("rating", "count"),  # Vote count
                R=("rating", "mean"),  # Average rating
                rating_std=("rating", "std"),  # Rating standard deviation
            )
            .reset_index()
        )

        # Step 2: Filter movies with insufficient ratings
        # Only consider movies with at least min_ratings to ensure statistical significance
        stats = stats[stats["v"] >= min_ratings]
        if stats.empty:
            return []

        # Step 3: Calculate IMDB Weighted Rating parameters
        # m: Minimum votes required (based on percentile of vote counts)
        # C: Global mean rating across all movies (baseline expectation)
        m, C = float(stats["v"].quantile(m_percentile)), self.global_mean

        # Step 4: Apply IMDB Weighted Rating Formula
        # WR = (v/(v+m)) * R + (m/(v+m)) * C
        # This formula balances movie quality (R) with vote quantity (v)
        # Movies with more votes get closer to their actual rating (R)
        # Movies with fewer votes get pulled toward the global average (C)
        stats["WR"] = (stats["v"] / (stats["v"] + m)) * stats["R"] + (
            m / (stats["v"] + m)
        ) * C

        # Step 5: Calculate Wilson Score Confidence Interval (Lower Bound)
        # This provides a conservative estimate of the "true" rating
        # z = 1.96 (95% confidence interval)
        # p_hat = proportion of "success" (rating/5.0, normalized to 0-1)
        # n = sample size (number of votes)
        z, p_hat, n = 1.96, stats["R"] / 5.0, stats["v"]

        # Wilson score lower bound formula:
        # (p_hat + z²/(2n) - z*sqrt((p_hat*(1-p_hat) + z²/(4n))/n)) / (1 + z²/n)
        # This gives us the lower bound of the confidence interval
        # Multiply by 5.0 to convert back to 1-5 rating scale
        stats["wilson_lower"] = (
            (
                p_hat
                + z**2 / (2 * n)
        ...
        # Step 6: Get top movies and merge with movie metadata
        # Sort by weighted rating (WR) in descending order
        top = stats.nlargest(limit, "WR").merge(self.movies, on="movieId")

        # Step 7: Format results into standardized dictionary structure
        # Each movie includes both raw statistics and calculated metrics
        result = []
            ...
            result.append(
                {
                    ...
                    "genres": str(r["genres"]),
                    # Rating statistics
                    "rating_mean": round(float(r["R"]), 3),  # Average rating
                    "rating_std": (  # Standard deviation
                        round(float(r["rating_std"]), 3)
                        if not pd.isna(r["rating_std"])
                        else 0.0
                    ),
                    "vote_count": int(r["v"]),  # Number of ratings
                    # Calculated metrics for ranking
                    "weighted_rating": round(float(r["WR"]), 3),  # IMDB weighted rating
                    "wilson_lower": round(
                        float(r["wilson_lower"]), 3
                    ),  # Wilson confidence lower bound
                    # Algorithm parameters (for transparency/debugging)
                    "m": round(m, 2),  # Minimum votes threshold
                    "C": round(C, 3),  # Global mean rating
                }
            )
        return result
    ...
    def calculate_movie_similarity(
        self, movie_id: int, method: str = "genre", limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for other_user in user_item_matrix.index:
                if other_user != user_id:
                    other_ratings = user_item_matrix.loc[other_user].values.reshape(
                        1, -1
                    )
                    # Simple cosine similarity
                    dot_product = np.dot(user_ratings, other_ratings.T)[0][0]
                    norm_product = np.linalg.norm(user_ratings) * np.linalg.norm(
                        other_ratings
                    )
            ...
            movie_scores.sort(
                key=lambda x: (x["predicted_rating"], x["confidence"]), reverse=True
            )
    ...
    def _hybrid_recommendations(self, user_id: int, limit: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for rec in collab_recs:
                genre_score = self._calculate_genre_match_score(
                    rec["genres"], genre_prefs
                )
                rec["hybrid_score"] = (rec["predicted_rating"] * 0.7) + (
                    genre_score * 0.3
                )
    ...
    def _rating_similarity(self, movie_id: int, limit: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            target_users = set(
                self.ratings[self.ratings["movieId"] == movie_id]["userId"]
            )
            ...
            movie_scores = (
                similar_movies.groupby("movieId")
                    ...
                    result.append(
                        {
                            ...
                            "similarity_score": float(row["avg_rating"])
            ...
            result.sort(
                key=lambda x: (x["similarity_score"], x["common_raters"]), reverse=True
            )
            return result[:limit]
    ...
    def _hybrid_similarity(self, movie_id: int, limit: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for movie in combined.values():
                hybrid_score = (movie["genre_similarity"] * 0.6) + (
                    movie["rating_similarity"] * 0.4
                )
    ...
    def _calculate_genre_preferences(
        self, user_movies: pd.DataFrame
    ) -> Dict[str, float]:
    ...
    def _calculate_genre_match_score(
        self, movie_genres: str, user_genre_prefs: Dict
    ) -> float:
    ...
    def get_top_rated_movies(
        self, n: int = 20, min_ratings: int = 100, m_percentile: float = 0.7
    ) -> pd.DataFrame:
        """
        ...
        top_movies_list = self.get_top_movies(
            limit=n, min_ratings=min_ratings, m_percentile=m_percentile
        )
        ...
        df = df.rename(
            columns={"rating_mean": "avg_rating", "vote_count": "rating_count"}
        )
        ...
        return (
            df[expected_cols] if all(col in df.columns for col in expected_cols) else df
        )
    ...
    def get_most_popular_movies(self, n: int = 20, limit: int = None) -> pd.DataFrame:
        ...
        movie_popularity = (
            self.ratings.groupby("movieId").agg({"rating": ["count", "mean"]}).round(3)
        )
    ...
    def analyze_temporal_patterns(self) -> Dict[str, pd.DataFrame]:
        ...
        # Daily patterns
        daily_patterns = (
            ratings_df.groupby("day_of_week")
            .agg({"rating": ["count", "mean", "std"]})
            .round(3)
        )
