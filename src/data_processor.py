...
class DataProcessor:
    ...
    def clean_movies(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Step 1: Remove duplicate movies based on movieId
        # Keep the first occurrence of each movieId to maintain data consistency
        df = df.drop_duplicates(subset=["movieId"])
        if len(df) < initial:
            logger.info(f"    Removed {initial - len(df)} duplicate movies")

        # Step 2: Data quality validation and cleaning
        # Drop rows with missing critical identifiers (movieId or title)
        # These are essential for movie identification and cannot be imputed
        df = df.dropna(subset=["movieId", "title"])

        # Step 3: Handle edge cases in title data
        # Replace empty string titles with a placeholder to maintain data integrity
        # This prevents issues in downstream analysis and visualization
        df.loc[df["title"].str.strip() == "", "title"] = "Unknown Title"

        # Step 4: Handle missing genre information
        # Fill missing genres with standardized placeholder
        # This maintains consistency and prevents null-related errors
        df["genres"] = df["genres"].fillna("(no genres listed)")

        # Step 5: Optimize data types for memory efficiency and performance
        # Convert to appropriate data types to reduce memory usage
        # int32 is sufficient for movieId (supports up to 2.1B movies)
        # string dtype is more efficient than object for text data
    ...
    def clean_ratings(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Duplicates can occur due to data collection errors or user re-submissions
        # Keep only unique combinations of all columns
        df = df.drop_duplicates()
        if len(df) < initial:
            logger.info(f"    Removed {initial - len(df)} duplicate ratings")

        # Step 2: Handle missing critical identifiers
        # Remove rows with missing userId as user identification is essential
        # For missing movieId, use placeholder (-1) to maintain data integrity
        df = df.dropna(subset=["userId"])
        df.loc[df["movieId"].isna(), "movieId"] = -1  # Placeholder for missing movieId

        # Step 3: Validate rating values
        # Filter ratings to valid MovieLens range (0.5-5.0 in 0.5 increments)
        # This removes invalid ratings that could skew analysis results
        df = df[(df["rating"] >= 0.5) & (df["rating"] <= 5.0)]

        # Step 4: Optimize data types for memory efficiency and performance
        # Convert to appropriate numeric types to reduce memory usage
        # int32 supports up to 2.1B users/movies, float32 sufficient for ratings
        df["userId"] = df["userId"].astype("int32", errors="ignore")
        df["movieId"] = df["movieId"].astype("int32", errors="ignore")
        df["rating"] = df["rating"].astype("float32", errors="ignore")

        # Step 5: Handle timestamp data conversion and missing values
        # Convert Unix timestamps to datetime objects for time-based analysis
        # Use forward fill to handle missing timestamps (maintains temporal order)
    ...
    def process_data(
        ...
        if "timestamp" in ratings_enhanced.columns:
            ratings_enhanced["datetime"] = pd.to_datetime(
                ratings_enhanced["timestamp"], unit="s"
            )
    ...
    def calculate_movie_stats(
        ...
        stats = (
            ...
            .agg(
                {"rating": ["count", "mean", "std", "min", "max"], "userId": "nunique"}
            )
            .round(3)
        )
        stats.columns = [
    ...
    def calculate_user_stats(self, ratings_df: pd.DataFrame) -> pd.DataFrame:
        ...
        stats = (
            ...
            .agg(
                {"rating": ["count", "mean", "std", "min", "max"], "movieId": "nunique"}
            )
            .round(3)
        )
        stats.columns = [
    ...
    def create_time_features(self, ratings_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "timestamp" in enhanced_df.columns:
            enhanced_df["datetime"] = pd.to_datetime(
                enhanced_df["timestamp"], unit="s", errors="coerce"
            )
    ...
    def normalize_ratings(self, ratings_df: pd.DataFrame) -> pd.DataFrame:
        ...
        user_stats = (
            ratings_df.groupby("userId")["rating"].agg(["mean", "std"]).reset_index()
        )
        user_stats["std"] = user_stats["std"].fillna(
            1.0
        )  # Handle users with only one rating

        # Merge stats back
        normalized_df = normalized_df.merge(
            user_stats, on="userId", suffixes=("", "_user")
        )

        # Normalize ratings
        normalized_df["normalized_rating"] = (
            normalized_df["rating"] - normalized_df["mean"]
        ) / normalized_df["std"]
        normalized_df["normalized_rating"] = normalized_df["normalized_rating"].fillna(
            0.0
        )
