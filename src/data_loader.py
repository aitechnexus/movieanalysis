...
class DataLoader:
    ...
    def load_movies(self) -> pd.DataFrame:
        ...
        Raises:
            FileNotFoundError: If movies.csv file doesn't exist
        ...
        """
        movies_file = self.raw_dir / "movies.csv"
    ...
    def load_ratings(self) -> pd.DataFrame:
        ...
        Raises:
            FileNotFoundError: If ratings.csv file doesn't exist
        ...
        """
        ratings_file = self.raw_dir / "ratings.csv"
    ...
    def download_data(self, url: str) -> bool:
        """
    ...
    def validate_data(self, movies_df: pd.DataFrame, ratings_df: pd.DataFrame) -> None:
        """
        Validate the loaded data for completeness and correctness.

        Args:
            movies_df: Movies DataFrame to validate
            ratings_df: Ratings DataFrame to validate
        ...
        # Check required columns for movies
        required_movie_cols = ["movieId", "title", "genres"]
        missing_movie_cols = [
            col for col in required_movie_cols if col not in movies_df.columns
        ]
        if missing_movie_cols:
            raise ValueError(
                f"Missing required columns in movies data: {missing_movie_cols}"
            )

        # Check required columns for ratings
        ...
        if missing_rating_cols:
            raise ValueError(
                f"Missing required columns in ratings data: {missing_rating_cols}"
            )

        # Check rating values are in valid range (0.5 to 5.0)
        invalid_ratings = ratings_df[
            (ratings_df["rating"] < 0.5) | (ratings_df["rating"] > 5.0)
        ]
        if not invalid_ratings.empty:
            raise ValueError(
                f"Invalid rating values found: ratings must be between 0.5 and 5.0"
            )
