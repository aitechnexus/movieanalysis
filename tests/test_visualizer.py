...
class TestInsightsVisualizer:
    ...
    def test_plot_rating_distribution(
        self, visualizer: InsightsVisualizer, sample_ratings_data: pd.DataFrame
    ):
    ...
    def test_plot_data_integrity_validation(self, visualizer: InsightsVisualizer):
        ...
        top_movies = [
            {
                "title": "Movie with Very Long Title That Should Be Handled Properly",
                "weighted_rating": 4.999,
                "vote_count": 1,
            },
        ...
        )  # Reasonable minimum size for a plot
            ...
            "overall": [
                {
                    ...
                    "mean_rating": np.random.uniform(1, 5),
                }
    ...
    def test_plot_error_handling_with_invalid_data(
        self, visualizer: InsightsVisualizer
    ):
    ...
    def test_comprehensive_statistics_plot_validation(
        self, visualizer: InsightsVisualizer, movie_analyzer
    ):
    ...
    def test_advanced_heatmap_generation(
        self, visualizer: InsightsVisualizer, movie_analyzer
    ):
    ...
    def test_plot_user_activity(self, visualizer: InsightsVisualizer):
        ...
        user_stats = {
            "user_activity_distribution": {"light": 100, "moderate": 80, "heavy": 20},
            "total_users": 200,
    ...
    def test_create_correlation_heatmap(
        ...
        plot_path = visualizer.create_correlation_heatmap(
            movie_analyzer, method="pearson"
        )

        assert os.path.exists(plot_path)
        assert plot_path.endswith(".png")

    def test_plot_movie_similarity_network(
        self, visualizer: InsightsVisualizer, movie_analyzer
    ):
    ...
    def test_create_dashboard_summary(
        self,
        ...
        analysis_results = {
            ...
            "rating_trends": pd.DataFrame(
                {"year": [2020, 2021], "avg_rating": [4.1, 4.2]}
            ),
            "user_activity": pd.DataFrame(
                {"userId": [1, 2], "rating_count": [100, 80]}
            ),
        }
    ...
    def test_save_plot_with_custom_name(
        self, visualizer: InsightsVisualizer, sample_ratings_data: pd.DataFrame
    ):
    ...
    def test_plot_with_invalid_columns(self, visualizer: InsightsVisualizer):
        ...
        invalid_data = pd.DataFrame(
            {"wrong_column": [1, 2, 3], "another_wrong": [4, 5, 6]}
        )
    ...
    def test_different_plot_formats(
        ...
        rating_dist = {
            ...
            "statistics": {"mean": 3.75, "median": 4.0, "std": 1.2},
        }
    ...
    def test_interactive_plot_creation(self, visualizer: InsightsVisualizer):
        ...
        plot_path = visualizer.create_interactive_plot(
            interactive_data, x="x", y="y", color="category"
        )
    ...
    def test_batch_plot_generation(
        self,
        visualizer: InsightsVisualizer,
        sample_movies_data: pd.DataFrame,
        sample_ratings_data: pd.DataFrame,
