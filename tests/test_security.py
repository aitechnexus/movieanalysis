...
class TestAPISecurity:
    ...
    def test_cors_security_configuration(self, client):
        ...
        if cors_headers["Access-Control-Allow-Origin"]:
            ...
            assert cors_headers["Access-Control-Allow-Origin"] != "*" or app.config.get(
                "TESTING", False
            )
    ...
    def test_http_security_headers(self, client):
        ...
        present_headers = [
            header for header in security_headers if header in response.headers
        ]
    ...
    def test_input_size_limits(self, client):
        ...
        response = client.post(
            "/api/dataset/load-url", json=large_json, content_type="application/json"
        )
    ...
    def test_api_versioning_security(self, client):
        ...
        for endpoint in version_endpoints:
            ...
            if response.status_code == 200:
                content = response.get_data(as_text=True)
                assert (
                    "version" not in content.lower() or "error" not in content.lower()
                )
...
class TestDataValidationSecurity:
    ...
    def test_numeric_input_validation(self):
        ...
        for value in extreme_values:
            try:
                ...
                elif isinstance(value, int) and abs(value) < 1000:  # Reasonable limit
                    limit_value = value
                else:
                    limit_value = 10  # Default safe value

                result = analyzer.get_top_movies(limit=limit_value)
    ...
    def test_string_input_sanitization(self):
        ...
        for i, dangerous_string in enumerate(dangerous_strings):
            try:
                # Test string processing methods
                # This assumes clean_movies exists and handles title data
                test_data = pd.DataFrame(
                    [{"movieId": i + 1, "title": dangerous_string, "genres": "Action"}]
                )
                result = processor.clean_movies(test_data)

                # Should handle dangerous input without crashing
                # Note: clean_movies is for data quality, not security sanitization
                if len(result) > 0:
                    processed_title = result.iloc[0].get("title", "")
                    # Should preserve the data (clean_movies doesn't sanitize, just cleans data quality)
                    assert isinstance(processed_title, str)
                    assert len(processed_title) > 0  # Should not be empty
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for path in dangerous_paths:
                try:
                    ...
                    # Mock the output directory to use dangerous path (convert to Path object)
                    try:
                        dangerous_path_obj = Path(path)
                        with patch.object(visualizer, "output_dir", dangerous_path_obj):
                        ...
                        # Path creation might fail for invalid paths
                        pass

                except (ValueError, OSError, PermissionError, TypeError):
