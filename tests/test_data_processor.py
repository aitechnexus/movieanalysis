...
class TestDataProcessor:
    ...
    def test_extract_genres(self, data_processor: DataProcessor, sample_movies_data: pd.DataFrame):
        ...
        assert len(genres_df) > len(sample_movies_data)  # Should have more rows due to genre splitting

    def test_calculate_movie_stats(self, data_processor: DataProcessor, 
                                 sample_movies_data: pd.DataFrame, 
                                 sample_ratings_data: pd.DataFrame):
        """Test movie statistics calculation"""
        stats_df = data_processor.calculate_movie_stats(sample_movies_data, sample_ratings_data)
    ...
    def test_calculate_user_stats(self, data_processor: DataProcessor, sample_ratings_data: pd.DataFrame):
        """Test user statistics calculation"""
        stats_df = data_processor.calculate_user_stats(sample_ratings_data)
        
        expected_columns = [
            'userId', 'avg_rating', 'rating_count', 'rating_std',
    ...
    def test_create_time_features(self, data_processor: DataProcessor, sample_ratings_data: pd.DataFrame):
        """Test time feature creation"""
        # Ensure timestamp is datetime
        sample_ratings_data['timestamp'] = pd.to_datetime(sample_ratings_data['timestamp'])
    ...
    def test_normalize_ratings(self, data_processor: DataProcessor, sample_ratings_data: pd.DataFrame):
    ...
    def test_process_data_pipeline(self, data_processor: DataProcessor,
                                 sample_movies_data: pd.DataFrame,
                                 sample_ratings_data: pd.DataFrame):
        """Test complete data processing pipeline"""
        processed_data = data_processor.process_data(sample_movies_data, sample_ratings_data)
        
        # Check that all expected keys are present
        expected_keys = [
            'movies_clean', 'ratings_clean', 'movie_stats', 'user_stats',
            'genres_df', 'ratings_enhanced'
        ]
    ...
    def test_missing_columns_handling(self, data_processor: DataProcessor):
        ...
        invalid_ratings = pd.DataFrame({'user': [1, 2], 'movie': [1, 2], 'score': [4.0, 3.5]})
        
        with pytest.raises(KeyError):
            data_processor.process_data(invalid_movies, invalid_ratings)

    @pytest.mark.parametrize("method", ["iqr", "zscore", "isolation"])
    def test_outlier_detection_methods(self, data_processor: DataProcessor, method: str):
        ...
        outliers = data_processor.detect_outliers(test_data, 'rating_count', method=method)
