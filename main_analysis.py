...
class MovieLensAnalysisPipeline:
    ...
    def __init__(self, data_source: str = "grouplens", dataset: str = "ml-25m"):
        ...
        logger.info(
            f"Initialized pipeline with source={data_source}, dataset={dataset}"
        )

    def run_full_analysis(self):
        start_time = datetime.now()
        logger.info("=" * 80)
        logger.info("Starting MovieLens Data Analysis Pipeline")
        logger.info("=" * 80)
        try:
            ...
            logger.info(
                f"✓ Loaded {len(movies_df):,} movies and {len(ratings_df):,} ratings"
            )
            ...
            pd.DataFrame(genre_stats["overall"]).to_csv(
                export_dir / "genre_statistics.csv", index=False
            )
            pd.DataFrame(time_series["monthly"]).to_csv(
                export_dir / "monthly_trends.csv", index=False
            )
...
def main():
    ...
    parser.add_argument(
        "--source", choices=["grouplens", "huggingface"], default="grouplens"
    )
